...
def format_df(raw, name):
    ...
    df = states_df.melt(id_vars = 'Province_State',
                        value_vars = [i for i in states_df.columns if i.endswith('20')],
                        var_name = 'Date',
                        value_name = name)
    
    df = df.pivot_table(index='Date', columns='Province_State', values = name).reset_index()
    df['Date'] = pd.to_datetime(df.Date)
...
confirmed.US.diff().resample('W', closed='left', label='left').mean().plot(ax = ax, label='Confirmed')
deaths.US.diff().resample('W', closed='left', label='left').mean().plot(ax = ax, label='Deaths')
ax.legend()
...
popdf['Country'] = popdf.Country.replace({'United States':'US', 'South Korea':'Korea, South'})
...
chart_data = df[['USA','Spain','France','United Kingdom','Brazil','Italy','Canada','Germany','Russia','India','Japan','South Korea']]
